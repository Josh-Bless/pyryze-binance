...
public class TestQueryUniversalTransfer {
    ...
    public void testQueryUniversalTransferWithoutType() {
        ...
        assertThrows(BinanceConnectorException.class, () -> client.createWallet().queryUniversalTransfer(parameters).block());
    }
    ...
    public void testQueryUniversalTransfer() {
        ...
        SpotClientImpl client = new SpotClientImpl(MockData.API_KEY, MockData.SECRET_KEY, baseUrl);
        String result = client.createWallet().queryUniversalTransfer(parameters).block();
